        ...
        {isOpen && (
          ...
          >
            <div className="p-6 border-b border-gold/15 flex justify-between items-center">
              ...
              <button onClick={onClose} className="text-gray-400 hover:text-gold transition-colors"><X size={28} /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {cart.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <button onClick={onClose} className="mt-4 text-gold hover:underline">Continue Shopping</button>
                </div>
              ) : (
                  ...
                  {cart.map((item, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-400 text-sm">{item.sizeLabel || `${item.size}ml`} × {item.quantity}</p>
                        <p className="text-gold text-lg font-semibold">{formatCurrency(item.price * item.quantity)}</p>
                        <div className="flex items-center gap-2 mt-2">
                          <button onClick={() => updateCartItemQuantity(item.id, -1)} className="w-7 h-7 border border-gold/30 text-white rounded flex items-center justify-center hover:bg-gold hover:text-black transition-all"><Minus size={14} /></button>
                          <span className="text-white text-sm w-6 text-center">{item.quantity}</span>
                          <button onClick={() => updateCartItemQuantity(item.id, 1)} className="w-7 h-7 border border-gold/30 text-white rounded flex items-center justify-center hover:bg-gold hover:text-black transition-all"><Plus size={14} /></button>
                        </div>
                      </div>
                      <motion.button
                        whileTap={{ scale: 0.8 }}
